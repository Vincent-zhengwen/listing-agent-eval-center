...
商品上架内容生成 Agent 评测中心是一个诊断式 Eval Harness。它不是只看一个总分的后台，而是把真实上架 case、Agent 运行过程、生成结果、grader evidence 和人工审核组织成一个可回归的质量闭环。

## 为什么需要它

商品上架内容生成 Agent 的失败不是单一问题。它可能是 source facts 缺失、标题事实漂移、主图重复、详情页说服力不足、B2B 供货话术泄漏，也可能是工具调用或持久化失败。
